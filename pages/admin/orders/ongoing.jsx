...
const ongoingOrders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const { data } = await axios.get("/api/admin/orders/ongoing");
  ...
  return (
    <Layout title="Order History" bgColor={`bg-[#f5f5f5]`}>
      ...
      <PageTitle title="Ongoing Orders" />
      <Logout />
      {loading ? (
        <div>Loading...</div>
      ) : error ? (
        ...
        <Empty title="No Records Found." text="No orders have been made yet" />
      ) : (
        <div className={style.tableContainer}>
          <Table orders={orders} admin />
        </div>
      )}
